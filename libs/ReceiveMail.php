...
class receiveMail
{
	...
	function receiveMail($username,$password,$mailserver='localhost', $port = '993') //Constructure
	{
		...
		$this->host 			=	rtrim($strConnect, 'INBOX');
	}
	...
	function getFolders() {
		...
		$folders = imap_list($this->marubox, $this->host, "*");
		foreach ($folders as $folder) {
			$result[] = ltrim($folder, $this->host);
		}
		return $result;
	}
	/**
	*	$page = halaman
	*	$data = show email / page
	**/
	function getHeader($page = 1,$data = 10) {
		if(!$this->marubox)
			return false;

		$numMessages = imap_num_msg($this->marubox);
		if($numMessages < $data) {
			$count = $numMessages;
		} else {
			$count = $page * $data;
		...
		for ($i = $numMessages; $i > ($numMessages - $count) ; $i--) { 
			$header = imap_header($this->marubox, $i);	
			$from = $header->from[0];

			$details[] = array(
				'from' => (empty($from->personal)) ? $from->mailbox : $from->personal,
				'subject' => $header->subject,
				'date' => explode(' ', $header->MailDate),
				'email' => $from->mailbox.'@'.$from->host,
				'unseen' => $header->Unseen,
				'uid' => imap_uid($this->marubox, $i),
				);
		}
		return $details;
	}
